...
# 第19-52行，修改为：
def binary_cross_entropy_with_hard_negative_mining(logits, labels, weights, batch_size, num_hard=2):
    classify_loss = nn.BCEWithLogitsLoss(reduction='mean')  # 改用BCEWithLogitsLoss
    
    # 注意：不再需要sigmoid，直接用logits
    logits_flat = logits[:, 0].view(-1, 1)
    pos_idcs = labels[:, 0] == 1
    
    pos_logits = logits_flat[pos_idcs, 0]  # 改用logits而非probs
    pos_labels = labels[pos_idcs, 0]
    
    neg_idcs = (labels[:, 0] == 0) & (weights[:, 0] != 0)
    neg_logits = logits_flat[neg_idcs, 0]  # 改用logits而非probs
    neg_labels = labels[neg_idcs, 0]
    
    if num_hard > 0:
        # OHEM需要用概率，所以这里临时sigmoid一下用于选择
        with torch.no_grad():
            neg_prob_for_select = torch.sigmoid(neg_logits)
        neg_prob_for_select, neg_labels_new = OHEM(neg_prob_for_select, neg_labels, num_hard * len(pos_logits))
        # 根据选择的索引获取对应的logits
        _, idcs = torch.topk(torch.sigmoid(neg_logits), min(num_hard * len(pos_logits), len(neg_logits)))
        neg_logits = neg_logits[idcs]
        neg_labels = neg_labels[idcs]
    ...
    if len(pos_logits) > 0:
        ...
        # 计算准确率时需要sigmoid
        with torch.no_grad():
            pos_correct = (torch.sigmoid(pos_logits) >= 0.5).sum()
        pos_total = len(pos_logits)
    else:
        cls_loss = 0.5 * classify_loss(neg_logits, neg_labels.float())
    
    # 计算负样本准确率
    with torch.no_grad():
        neg_correct = (torch.sigmoid(neg_logits) < 0.5).sum()
    neg_total = len(neg_logits)
