...
class BboxReader(Dataset):
    def __init__(self, data_dir, set_name, cfg, mode='train', split_combiner=None):
        ...
        if mode != 'test':
            self.filenames = [f for f in self.filenames if os.path.exists(os.path.join(data_dir, '%s_bboxes.npy' % f)) and os.path.exists(os.path.join('/media/yangao/SANet/NoduleNet-master/data/LUNA16/allset/combined', '%s.mhd' % f))]
