...
def eval(net, dataset, save_dir=None):
    ...
    for i, (input, truth_bboxes, truth_labels, image) in enumerate(dataset):
        try:
            ...
            print(f"处理第 {i+1} 张CT图像")    
            print(f'CT图像的pid: {pid}, CT图像的形状:{image.shape}\n')
    ...
    true_positives, total_nodules = noduleCADEvaluation('annotations/new_annotations.csv',
                    'annotations/new_annotations_excluded.csv',
                    dataset.set_name, ensemble_submission_path, os.path.join(eval_dir, 'ensemble'))
    
    # 输出True Positives和Total Number of Nodules
    if true_positives is not None and total_nodules is not None:
        print(f"\n===== 敏感性计算关键指标 =====")
        print(f"True Positives (检测到的真正例): {true_positives}")
        print(f"Total Number of Nodules (总结节数): {total_nodules}")
        if total_nodules > 0:
            sensitivity = true_positives / total_nodules
            print(f"Sensitivity (敏感性): {sensitivity:.6f}")
        print(f"=============================\n")
    else:
        print("无法获取敏感性计算指标")
