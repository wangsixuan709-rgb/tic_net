...
class Logger(object):
    def __init__(self, logfile):
        ...
        # 写入开始时间
        self.log.write(f"\n{'='*60}\n")
        self.log.write(f"日志开始时间: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        self.log.write(f"{'='*60}\n")
        self.log.flush()

    def write(self, message):
        # 添加时间戳到重要消息
        if any(keyword in message for keyword in ['敏感性指标', 'True Positives', 'Total Number', 'Sensitivity']):
    ...
    def close(self):
        # 写入结束时间
        self.log.write(f"\n{'='*60}\n")
        self.log.write(f"日志结束时间: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        self.log.write(f"{'='*60}\n")
        self.log.close()
...
def py_nms(dets, thresh):
    ...
    z, y, x, d, h, w = dets[:, 1],dets[:, 2],dets[:, 3],dets[:, 4],dets[:, 5],dets[:, 6]
    scores = dets[:, 0]
...
def bboxes_masks2masks(coord_boxes, masks, labels, reso, num_class=len(net_config['roi_names'])):
    """
...
def dice_score(y_pred, y_true, num_class=len(net_config['roi_names']), decimal=4):
    res = []
    for i in range(num_class):
        target = y_true == i
        pred = y_pred == i
...
def dice_score_seperate(y_pred, y_true, num_class=len(net_config['roi_names']), decimal=4):
    res = []
    for i in range(num_class):
        target = y_true[i]
        pred = y_pred[i]
...
def hausdorff_distance(y_pred, y_true, spacing=[1., 1., 1.], percent=0.95, num_class=len(net_config['roi_names']), decimal=4):
    """
...
def annotation2multi_mask(mask):
    ...
    for i, roi in enumerate(net_config['roi_names']):
        if roi in mask:
...
def annotation2masks(mask):
    ...
    masks = np.zeros([len(net_config['roi_names']), D, H, W])
    for i, roi in enumerate(net_config['roi_names']):
        if roi in mask:
...
def multi_mask2onehot(mask):
    D, H, W = mask.shape
    onehot_mask = np.zeros((len(net_config['roi_names']) + 1, D, H, W))
...
if __name__ == '__main__':
    path = "/home/ligen/PreprocessedImage"
    filecount(path)
    res = find_all_files(path, '_bboxes.npy')
    print(len(res))
    # uid_list = []
    # for uid in res:
    #     uid = uid.split('/')[-1][:-11]
    #     uid_list.append(uid)
    # print(uid_list)
    # with open('/home/ligen/Nodule-Detection/split/all.csv', 'w') as csvfile:
    #     writer = csv.writer(csvfile)
