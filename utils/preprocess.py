...
# sys.path.append('/media/yangao/SANet/TiCNet')
...
def preprocess(params):
    ...
    save_path = os.path.join(save_dir, f'{pid}_origin.npy')
    
    if os.path.exists(save_path):
        print(f"Skipping {pid}, file already exists.",flush=True)
        return 
    # print(f'Preprocessing {pid}...')
    
    lung_mask_path = os.path.join(lung_mask_dir, f'{pid}.mhd')
    img_path = os.path.join(img_dir, f'{pid}.mhd')
    # print(img_path)
    # 检查文件是否存在
    if not os.path.exists(lung_mask_path):
        # print(f'[Warning] Lung mask file not found: {lung_mask_path}')
        return
    if not os.path.exists(img_path):
        # print(f'[Warning] Image file not found: {img_path}')
        return
    raw_path = img_path.replace('.mhd', '.raw')
    if not os.path.exists(raw_path):
        print(f"Skipping {pid}, missing .raw file: {raw_path}", flush=True)
        return
    print('123123')

    lung_mask, _, _ = load_itk_image(lung_mask_path)
